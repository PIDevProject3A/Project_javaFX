...
public class PrevueCollectionDAO {
    ...
    public void insert(PrevueCollection p) throws SQLException {
        String sql = """
            INSERT INTO prevue_collection (type_collection, quantite, statut, waste_collection_id, collection_date, unit)
            VALUES (?,?,?,?,?,?)
            """;
        try (Connection conn = DatabaseConnection.getConnection();
    ...
    public void update(PrevueCollection p) throws SQLException {
        String sql = """
            UPDATE prevue_collection
               SET type_collection=?, quantite=?, statut=?, waste_collection_id=?, collection_date=?, unit=?
             WHERE id=?
            """;
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
